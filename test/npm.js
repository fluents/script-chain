...
test('npm auto adds doubledash/groups', t => {
  const scripts = new ScriptChain().add().npm('diggy').arg('--eh')
...
test('using npm created multiple grouped flags', t => {
  ...
  // log.bold('npm').stringify({scripts, script}).echo()

  const data = Object.values(scripts.scripts).pop()
  t.pass(fosho(data.get('npm')).eq('diggy'))
})

test('script-chain creates expected result', t => {
  const scripts = new ScriptChain()
  ...
  const expected = [
    {
      bin: 'npm',
      args: ['run-script', 'diggy', '--', '--env.zoolala=aoao', '-e'],
      env,
    },
  ]
